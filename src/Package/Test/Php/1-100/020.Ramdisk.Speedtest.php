...
echo 'Ramdisk write time: ' . time_format($duration_write, '') . '; size: ' . size_format(($size/$duration_write)) . ' /sec'. PHP_EOL;
$start = microtime(true);
$read = file_get_contents($ramdisk);
$duration_read = microtime(true) - $start;
echo 'Ramdisk read time: ' . time_format($duration_read, '', true) . ' ' . size_format(($size/$duration_read)) . ' /sec' . PHP_EOL;
